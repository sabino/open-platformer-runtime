...
- Standalone .NET asset tools validate the current `manifest.json` graph, core level `105`/`1CB` metadata, player OAM table exports, palette dimensions, tileset/sprite GFX uploads, audio bank hashes, PNG/WAV headers, and key Map16 projection anchors without invoking Python. The native extractor tool also verifies that generated GFX32/GFX33, raw SPC banks, global Map16, first-level level/sprite/exit metadata, and BRR sample preview WAVs still byte-match direct C# extraction/decoding from the original ROM.
