...
- A normal-play status bar shows score, lives, normal coins, dragon coins, level timer, current level, course-clear state, and the first game-over flow; the timer now triggers a first time-up death/restart path, and the same fields are exposed through RCON for headless verification.
