...
- Runtime coin pickup state is derived from imported Map16 coin markers, including normal coins and dragon coins, with named internal APU SFX probes on collection and extra-life events plus a first native-style floating score popup layer for coin, item, stomp, star, and 1UP rewards.
...
To record and compare without touching save states, use `tools/run-native-input-recording-wayland.sh`. It records native input from boot into timestamped files such as `generated/smw/recordings/native-YYYYMMDD-HHMMSS-full.input`, writes matching marker files, slices a timestamped level-start input from the `K` marker or automatic level-start marker, and refreshes stable aliases at `latest-native-full.input`, `latest-native-full.input.markers`, `latest-level-start.input`, and `latest-level-start.frame`. Use the stable aliases for normal harness runs and keep the timestamped recordings as archival evidence when a baseline matters.

Use `tools/run-native-input-wayland.sh path/to/full-route.input` for native cold-boot playback. Use `tools/run-recording-compare-wayland.sh --input path/to/full-route.input --level-start-frame N` to open native cold-boot playback next to Godot running the level-start slice, or `tools/run-recording-trace-compare.sh --input path/to/full-route.input --level-start-frame N --frames 600` to write native/Godot logs plus JSONL-normalized traces under `generated/smw/traces/` and report the first player position or power-up divergence. If `--level-start-frame` is omitted, the trace wrapper reads the marker file next to the input and aligns native comparison to the selected absolute frame while Godot consumes the sliced level-start input. `tools/slice-input-script.py` is the helper that turns the second signal, the level-start frame, into the Godot slice; the compare wrappers currently prepend two neutral Godot frames by default via `--godot-input-delay 2` to match the observed native/Godot input-latch phase.
...
Audio is silent by default for now. Pass `--smw-audio`, `--smw-audio=on`, or set `SMW_AUDIO=1` to enable the internal BRR/APU probe; `--smw-no-audio` and `SMW_AUDIO=0` keep it disabled.
