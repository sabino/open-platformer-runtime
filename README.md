...
That launches `/path/to/native-reference` visibly through its Wayland wrapper on workspace `6`. Press `K` in native SMW when the part you want starts, play normally, then press `Shift+F1` to save `save1.sav` or finish a level so `SavePlaythrough` writes `playthrough/*.sav`. After you close the native window, the wrapper converts the newest native snapshot joypad log into `generated/smw/recordings/latest-native-recording.input` using `tools/convert-native-snapshot-input.py`. Replay the captured input in Godot with `tools/run-recorded-input-wayland.sh`.
