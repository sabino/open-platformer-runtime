...
- Actors-on collision has a first frame-crossing stomp resolver for Rex/Koopa style bodies and a narrower Banzai Bill gameplay hitbox, with headless coverage proving Mario can pass under the first Banzai while it remains active nearby. Full actors-on course-clear traversal is still pending.
...
- The headless gate now includes an actor-off `--smw-autoplay=explore` traversal that reaches the imported level `105` goal and enters the course-clear walkout, providing an end-to-end regression for first-level layout, camera, timer, and deterministic control. Actors-on traversal is still a work-in-progress while enemy behavior is ported.
