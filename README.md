...
- ROM validation and deterministic extraction for level `105` by default. Native C# extractor slices now directly extract and verify core ROM assets used by the runtime, including GFX32/GFX33, raw SPC upload banks, global Map16, level `105`/`1CB` headers, object stream lengths/raw bytes, sprite stream positions, layer-2 background source metadata, the level `105` screen-07 pipe route into `1CB`, BRR-decoded audio preview WAVs, and Mario OAM/palette metadata.
...
- Standalone .NET asset tools validate the current `manifest.json` graph, core level `105`/`1CB` metadata, player OAM table exports, palette dimensions, tileset/sprite GFX uploads, audio bank hashes, PNG/WAV headers, and key Map16 projection anchors without invoking Python. The native extractor tool also verifies that generated GFX32/GFX33, raw SPC banks, global Map16, first-level level/sprite/exit metadata, BRR sample preview WAVs, and generated `PlayerGFXRt` OAM/palette tables still byte-match direct C# extraction/decoding from the original ROM.
...
For deterministic input checks, pass `--smw-input-script=/path/to/script` or `--smw-autoplay=explore`. Each script non-comment line starts with a frame count followed by held inputs separated by spaces, commas, colons, or semicolons, for example `8 right run` or `4 Right,Y`. Supported gameplay tokens are `left`, `right`, `down`, `jump`/`B`, `spin`/`A`, and `run`/`X`/`Y`; native `.input` directives and menu/shoulder tokens such as `Start`, `Select`, `Up`, `L`, and `R` are accepted as no-ops for compatibility with the current `smw/` TAS converter output. Normal play maps keyboard arrows/A/D/S plus Z/Space, X, Shift/C, and Enter, and also maps a standard gamepad: D-pad/left stick for movement, A for jump, B for spin jump, X or right shoulder for run/fire, and Start for pause/menu start.
