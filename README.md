...
During a debug RCON run, `tools/smw-rcon.sh snapshot res://generated/smw/captures/name.png` saves the current Godot viewport immediately in visible runs, while `capture <path> <frames>` schedules a capture after one or more process frames. The same RCON channel can toggle inspection helpers live: `overlays on`, `actors off`, and `god on` are useful before positioning Mario for a screenshot or frame-step probe. Use `tile` or `tile <world-x> <world-y>` to query the Map16/collision role under a point, `near <radius>` to list nearby runtime sprite actors, and `oam [tag]` to dump Mario's current native pose/OAM descriptor mapping. Use `ground on|off` to force the grounded flag for a controlled probe, and `trace <frames> [inputs...] [tag=name]` to step a paused run while logging per-frame position, speed, camera, foot tile, and nearest actor data.
