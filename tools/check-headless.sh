...
RESTART_OAM_COMMAND_FILE="$(mktemp)"
...
EOF
cat >"$RESTART_OAM_COMMAND_FILE" <<'EOF'
pause
oam before_restart
restart
oam after_restart
...
"$GODOT_BIN" --headless --path . --quit-after 4 --smw-test-autostart --smw-debug-command-file="$RESTART_OAM_COMMAND_FILE" --smw-no-audio 2>&1 | tee "$LOG_FILE"
grep -q "smw-debug: command_file=$RESTART_OAM_COMMAND_FILE" "$LOG_FILE"
grep "smw-debug-player-oam: tag=before_restart" "$LOG_FILE" | grep -q "slot0:.*visible=1"
grep "smw-debug-player-oam: tag=before_restart" "$LOG_FILE" | grep -q "slot1:.*visible=1"
grep "smw-debug-player-oam: tag=after_restart" "$LOG_FILE" | grep -q "slot0:.*visible=1"
grep "smw-debug-player-oam: tag=after_restart" "$LOG_FILE" | grep -q "slot1:.*visible=1"
! grep -q "ObjectDisposedException" "$LOG_FILE"
