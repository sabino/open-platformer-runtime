...
grep -q "smw-runtime: sprite_stomp level=105 sprite=95" "$LOG_FILE"
grep -q "smw-runtime: course_clear level=105 walkout=right" "$LOG_FILE"
...
grep "smw-debug-state: tag=step_done" "$LOG_FILE" | grep -q "max_x=5106.00"
grep "smw-debug-state: tag=step_done" "$LOG_FILE" | grep -q "actor_event=stomp:95:dead"
