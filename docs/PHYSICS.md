...
- Runtime slope collision is a temporary geometric bridge. Only imported diagonal pipe cells, `left_diagonal_ledge_edge`, and `steep_right_slope_surface` become slope surfaces; ledge/slope fill cells stay solid so Mario does not treat the interior of a slope object as the walkable line. These surfaces are now emitted per Map16 tile rather than as long connected-component averages, which avoids false collision lines when adjacent or overlapping slope objects touch.
