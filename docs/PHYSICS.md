...
- The first remaining player divergence is horizontal timing/position around the first held-right segment. Native and Godot traces now include player subposition and subspeed. After switching to native-style move-before-physics ordering, the first divergence is at index `43`: native reports `x=16, subx=128, xs=6`, while Godot reports `x=17.31, subx=80, xs=9`. The next task is to isolate whether this is input-latch phase, a still-wrong horizontal acceleration table branch, or a missing native startup/control-lock condition.
