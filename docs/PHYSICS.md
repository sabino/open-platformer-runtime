...
- Enemy sync must be treated as part of the same trace surface. Godot live traces now include nearest actor id/state/position/speed/active fields, and the native reference trace exposes the nearest normal sprite slot/id/status/position/speed/subpixel fields. Once Mario's anchor is corrected, these fields should be promoted from diagnostics to comparator checks for spawn position, activation timing, walking/falling speed, and hitbox contact order.
- The first enemy diagnostic trace is already useful: on the same early level-start frames, native reports nearest sprite `0xBD` around `x=162,y=325` with raw speed `0xE0/0x29`, while Godot reports nearest actor `0xBD` around `x=207,y=193` with small float speeds. That is a separate actor spawn/anchor/speed mismatch to resolve after the player coordinate basis is made consistent.
