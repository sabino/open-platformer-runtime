...
- Normal flat-ground horizontal caps currently use the first entries from `kHandlePlayerPhysics_DATA_00D535`: walking `0x14`, running `0x24`, and full P-meter sprinting `0x30`.
...
- Flat-ground turn-around acceleration now uses the native `kHandlePlayerPhysics_MarioAccel` walk/run entries `0x0280` and `0x0500` instead of the earlier temporary `0x0600` shortcut.
