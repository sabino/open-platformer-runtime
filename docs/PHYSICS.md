...
- The first remaining player divergence is horizontal timing/position around the first held-right segment. In the current trace, native begins reporting right input one record after the Godot slice and native X remains `16` until frame `3076`, while Godot reaches `x=17.31` by its frame `42`. The next task is to determine whether this is a one/two-frame trace/input phase offset, native subpixel carry not yet exposed in the trace, or a real acceleration integration mismatch.
