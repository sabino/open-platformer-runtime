...
Runtime pipe note: pipe debug rectangles are rebuilt from imported screen-exit records and the corresponding generated vertical pipe top or horizontal pipe end Map16 placements. For level `105`, this puts the active pipe trigger on the screen `07` pipe instead of the old hardcoded screen `01` marker; for level `1CB`, the horizontal pipe end now has a left/right-triggered rectangle for the return screen exit.
