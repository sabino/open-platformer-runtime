...
GFX note: foreground/background and sprite VRAM atlases are resolved per level. Clean vanilla levels use the native upload tables, while Lunar Magic Super GFX bypass data is recognized through the table pointer at `$0FF7FF` and slot words for `FG3/BG1/FG2/FG1` and `SP4/SP3/SP2/SP1`. The importer can also resolve ExGFX pointer tables structurally, but generated validation assets are still produced from the verified vanilla ROM.
